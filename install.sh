...
#Setup boot options
echo "initramfs initramfs.img followkernel" >> /mnt/boot/config.txt
#Setup crypttab
...
#Backup boot parttion so removing kernel dosn't wipe all driver blobs
mkdir backup 
cp -r /mnt/boot/* backup/

#Forcefull remove the OpTiMiZeD kernel
xchroot /mnt xbps-remove -F rpi-kernel
...
#Generate initramfs
xchroot /mnt depmod $(ls -S /lib/modules/ | tail -1)
xchroot /mnt dracut -f /boot/initramfs.img --kver $(ls -S /lib/modules/ | tail -1)

#xchroot /mnt xbps-install -y rpi-base
